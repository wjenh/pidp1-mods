...
The pidp-1 must be in the stop state.

The *count* is always a decimal number.

Execution starts from the current pc.
This is equivalent to giving the *step* command *count* times.

The output file can then be disassembled by **disassembler** using *-Lmem* and has the format:
```
aaaaaa: iiiiii

where aaaaaa and iiiiii are 6 digit octal values, the address and the instruction.
```

Breakpoints and watchpoints will not be reported until this completes.
