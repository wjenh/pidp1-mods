...
255 intensity steps are used occurring at 33.33 millsecond intervals, a frame rate of 30 frames per second.
...
Initially, alpha blending is done of the two colors, but once the primary phosphor decays to nn insignificant
...
Each frame increments the lifetime after the alpha computations are done and if the 255 step limit has been reached
the point is marked as inactive and will no longer be drawn.

A point is inactivated if its intensity falls to 0 for the primary phosphor or to a very small value for the
secondary phosphor. This value is configrable in the code, as are many other values.
...
Here's a table of CPU load by program and by display emulator.
...
|------------|-----------|--------------------------|
| snowflake    | type30dpy | 107%   |
|              | p7sim-mod | 184%   |
|              | p7sim     | 284%   |
| spacewar48   | type30dpy | 108%   |
|              | p7sim-mod | 314%*  |
|              | p7sim     | 280%   |
| lines        | type30dpy | 107%   |
|              | p7sim-mod | 170%   |
|              | p7sim     | 285%   |
| type340lines | type30dpy | 117%   |
|              | p7sim-mod | 168%   |
|              | p7sim     | 304%** |

\* - this is an anomoly, but repeatable\
\** - p7sim can't handle the data rate type304lines generates, loses many pixels

T30dpy can also report some timing statistics.
Running type340lines it is rendering up to 4 million dots per second without any lost data.
...
o port - the port to connect to, default is 3400
